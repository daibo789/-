...
extension TimVideoCallViewController {
    func setupUI() {
        TimVideoCallViewController.renderViews = []
        ...
        TimTRTCVideoCall.shared.openCamera(frontCamera: true, view: localPreView)
    }
    ...
    func setupControls() {
        if hangup.superview == nil {
            ...
            hangup.rx.controlEvent(.touchUpInside).subscribe(onNext: {[weak self] in
                guard let self = self else {return}
                 TimTRTCVideoCall.shared.hangup()
                self.disMiss()
        ...
        if accept.superview == nil {
            ...
            accept.rx.controlEvent(.touchUpInside).subscribe(onNext: {[weak self] in
                guard let self = self else {return}
                TimTRTCVideoCall.shared.accept()
        ...
        if mute.superview == nil {
            ...
            mute.rx.controlEvent(.touchUpInside).subscribe(onNext: { [weak self] in
                guard let self = self else {return}
                TimTRTCAudioCall.shared.isMicMute = !TimTRTCAudioCall.shared.isMicMute
                 self.mute.setImage(UIImage(named: TimTRTCAudioCall.shared.isMicMute ? "ic_mute_on" : "ic_mute"), for: .normal)
                self.view.makeToast(TimTRTCAudioCall.shared.isMicMute ? "开启静音" : "关闭静音")
                }, onError: nil, onCompleted: nil, onDisposed: nil).disposed(by: disposebag)
            mute.isHidden = true
        ...
        if handsfree.superview == nil {
            ...
            handsfree.rx.controlEvent(.touchUpInside).subscribe(onNext: { [weak self] in
                guard let self = self else {return}
                TimTRTCAudioCall.shared.isHandsFreeOn = !TimTRTCAudioCall.shared.isHandsFreeOn
                self.handsfree.setImage(UIImage(named: TimTRTCAudioCall.shared.isHandsFreeOn ? "ic_handsfree_on" : "ic_handsfree"), for: .normal)
                self.view.makeToast(TimTRTCAudioCall.shared.isHandsFreeOn ? "开启免提" : "关闭免提")
                }, onError: nil, onCompleted: nil, onDisposed: nil).disposed(by: disposebag)
