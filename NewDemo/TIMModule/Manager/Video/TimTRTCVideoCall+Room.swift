...
extension TimTRTCVideoCall: TRTCCloudDelegate {
    ...
    func enterRoom() {
        ...
        param.sdkAppId = UInt32(TIM_SDKAPPID)
        param.userId = TimVideoCallUtils.shared.curUserId()
        ...
        if let del = delegate {
            for info in userVolumes {
                if let userId = info.userId {
                    ...
                    del.onUserVoiceVolume?(uid: TimVideoCallUtils.shared.curUserId(), volume: UInt32(info.volume))
                }
            }
        }
    }
