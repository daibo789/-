...
extension TimTRTCVideoCall {
    ...
    func handleCallModel(user: String,
        ...
        func syncInvitingList() {
            if let _ = curGroupID {
                ...
                filterUserIds = filterUserIds.filter { $0 != TimVideoCallUtils.shared.curUserId() }
        ...
        case .dialing:
            if (model.groupid != nil &&
                !model.invitedList.contains(TimVideoCallUtils.shared.curUserId())) { //群聊但是邀请不包含自己不处理
            ...
            if isOnCalling { // tell busy
                ...
                //被邀请方检查超时
                checkTimeID = TimVideoCallUtils.shared.generateRoomID()
    ...
    func sendModel(user: String, action: VideoCallAction,
        ...
        guard let data = TimVideoCallUtils.shared.callModel2Data(model: realModel) else {
            return
        }
    ...
    func checkTimeOut(uid: String, time: Double) {
        let checkID = TimVideoCallUtils.shared.generateRoomID()
