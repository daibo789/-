...
@objc public class TimTRTCAudioCall: NSObject,
                                  TimITRTCAudioCallInterface,
                                  TIMMessageListener {
    @objc public static let shared = TimTRTCAudioCall()
    ...
    @objc public func login(sdkAppID: UInt32,
        ...
        TIMManager.sharedInstance()?.login(loginParam, succ: {
            success()
            //设置 APNS
            if let deviceToken = TIMappUtils.shared.appDelegate.deviceToken {
                let param = TIMTokenParam.init()
                param.busiId = UInt32(TIM_SDKBusiId)
    ...
    internal func invite(userIds: [String], type: AudioCallType = .audio, groupID: String? = nil) {
        if !isOnCalling { //首次邀请
            curCallID = TimAudioCallUtils.shared.generateCallID()
            curRoomID = TimAudioCallUtils.shared.generateRoomID()
            curGroupID = groupID
            curType = type
            isOnCalling = true
