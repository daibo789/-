...
public class TimTRTCVideoCall: NSObject,
                            TimITRTCVideoCallInterface,
                            TIMMessageListener {
    @objc public static let shared = TimTRTCVideoCall()
    ...
    internal func invite(userIds: [String], type: VideoCallType, groupID: String? = nil) {
        if !isOnCalling { //首次邀请
            curCallID = TimVideoCallUtils.shared.generateCallID()
            curRoomID = TimVideoCallUtils.shared.generateRoomID()
            curGroupID = groupID
            curType = type
            isOnCalling = true
