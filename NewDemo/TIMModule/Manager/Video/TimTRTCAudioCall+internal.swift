...
extension TimTRTCAudioCall {
    ...
    func handleCallModel(user: String,
        ...
        func syncInvitingList() {
            if let _ = curGroupID {
                ...
                filterUserIds = filterUserIds.filter { $0 != TimAudioCallUtils.shared.curUserId() }
        ...
        case .dialing:
            if (model.groupid != nil &&
                !model.invitedList.contains(TimAudioCallUtils.shared.curUserId())) { //群聊但是邀请不包含自己不处理
            ...
            if isOnCalling { // tell busy
                ...
                //被邀请方检查超时
                checkTimeID = TimAudioCallUtils.shared.generateRoomID()
    ...
    func sendModel(user: String, action: AudioCallAction,
        ...
        guard let data = TimAudioCallUtils.shared.callModel2Data(model: realModel) else {
            return
        }
    ...
    func checkTimeOut(uid: String, time: Double) {
        let checkID = TimAudioCallUtils.shared.generateRoomID()
