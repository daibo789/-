...
import Foundation
extension TimVideoCallViewController: UICollectionViewDelegate, UICollectionViewDataSource,
                                   UICollectionViewDelegateFlowLayout {
    
    func resetWithUserList(users: [TimVideoCallUserModel], isInit: Bool = false) {
        resetUserList()
        let usersFilter = users.filter {
            $0.userId != TimVideoCallUtils.shared.curUserId()
        }
    ...
    func resetUserList() {
        if let sponsor = curSponsor {
            ...
            var curUser = TimVideoCallUserModel()
            if let name = TimProfileManager.shared.curUserModel?.name,
                let avatar = TimProfileManager.shared.curUserModel?.avatar,
                let userId = TimProfileManager.shared.curUserModel?.userId {
                curUser.name = name
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "VideoCallUserCell", for: indexPath) as! TimVideoCallUserCell
        if (indexPath.row < avaliableList.count) {
            let user = avaliableList[indexPath.row]
            cell.userModel = user
            if user.userId == TimVideoCallUtils.shared.curUserId(){
            ...
            cell.userModel = TimVideoCallUserModel()
        }
        return cell
    }
    ...
    /// - Parameter user: 用户信息
    func enterUser(user: TimVideoCallUserModel) {
        if user.userId != TimVideoCallUtils.shared.curUserId() {
            let renderView = VideoRenderView()
            renderView.userModel = user
            TimTRTCVideoCall.shared.startRemoteView(userId: user.userId, view: renderView)
            TimVideoCallViewController.renderViews.append(renderView)
    ...
    func leaveUser(user: TimVideoCallUserModel) {
        TimTRTCVideoCall.shared.stopRemoteView(userId: user.userId)
        TimVideoCallViewController.renderViews = TimVideoCallViewController.renderViews.filter {
    ...
    func updateUser(user: TimVideoCallUserModel, animate: Bool = false) {
        if let index = userList.firstIndex(where: { (model) -> Bool in
            model.userId == user.userId
        }) {
            userList.remove(at: index)
    ...
    func updateLayout() {
        ...
        if collectionCount == 2 {
            ...
            if let user = userFirst {
                if let firstRender = TimVideoCallViewController.getRenderView(userId: user.userId) {
