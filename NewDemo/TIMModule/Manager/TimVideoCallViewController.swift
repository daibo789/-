...
@objc public enum TimVideoCallState : Int32, Codable {
    case dailing = 0
    case onInvitee = 1
    case calling = 2
}
...
class TimVideoCallViewController: UIViewController {
    lazy var userList: [TimVideoCallUserModel] = []
    
    /// 需要展示的用户列表
    var avaliableList: [TimVideoCallUserModel] {
        get {
...
//            }
    ...
    let curSponsor: TimVideoCallUserModel?
    ...
    var curState: TimVideoCallState {
        didSet {
    ...
    init(sponsor: TimVideoCallUserModel? = nil) {
        curSponsor = sponsor
        if let _ = sponsor {
            curState = .onInvitee
        } else {
    ...
    deinit {
        TimTRTCVideoCall.shared.closeCamara()
    ...
    lazy var userCollectionView: UICollectionView = {
        ...
        user.register(TimVideoCallUserCell.classForCoder(), forCellWithReuseIdentifier: "VideoCallUserCell")
    ...
    func getUserById(userId: String) -> TimVideoCallUserModel? {
